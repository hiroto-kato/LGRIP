...
fun main (name, args) =
    if null args then (print helpMsg; OS.Process.success)
	     ...
	     val rest = L.tl args
	     val isTheorem =  if L.exists (fn str => LU.member str option1) rest orelse L.exists (fn str => L.exists (fn pattern => String.isSubstring pattern str) option2) rest 
			      then case LU.member "--lemma" rest of 
				       false => (case LU.member "--debug" rest orelse LU.member "-D" rest of
						     true => (case LU.member "--noIteration" rest orelse LU.member "-nI" rest of
								  true => (case L.exists (fn str => String.isSubstring "--depth=" str) rest orelse L.exists (fn str => String.isSubstring "-d=" str) rest of
								...
								| false => (case L.exists (fn str => String.isSubstring "--depth=" str) rest orelse L.exists (fn str => String.isSubstring "-d=" str) rest of
										true => (case Util.intStrL rest option2 of
											     0 => IndMain.procPrintDef filename
											   | depth => IndMain.procDepthPrintDef filename depth)
									      | false => IndMain.procPrintDef filename))
						   | false => (case LU.member "--noIteration" rest orelse LU.member "-nI" rest of
								   true => (case L.exists (fn str => String.isSubstring "--depth=" str) rest orelse L.exists (fn str => String.isSubstring "-d=" str) rest of
										true => (case Util.intStrL rest option2 of
