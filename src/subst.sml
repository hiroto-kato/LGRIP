...
fun match pattern term =
    let
        ...
        fun unionPair [] us = us
          | unionPair ts [] = ts
          | unionPair ((k,v)::ts) ((a,b)::us) = if k=a andalso v=b
                                                then unionPair ts ((a,b)::us)
                                                else unionPair ts ((k,v)::(a,b)::us)
        fun main [] sigma = SOME sigma
          | main ((T.Fun (f,ts),T.Fun (g,us))::rest) sigma = if f=g
                                                             then main (unionPair (decompose ts us) rest) sigma 
                                                             else NONE
          | main ((T.Fun _, T.Var _)::_) _ = NONE
          | main ((T.Var x, t0)::rest) sigma = case AL.find x sigma
                                                of NONE => main rest (valOf (AL.add (x,t0) sigma))
                                                 | SOME t1 => if t1=t0
                                                              then main rest sigma
                                                              else NONE
    ...
    let
        ...
        fun main nil sigma = SOME sigma
          ...
          | main ((T.Var x, t0)::rest) sigma =
            ...
            else if (AL.find x sigma) = (SOME t0)
            then main rest sigma
            else NONE
          | main ((T.Fun (f,ts),T.Fun (g,us))::rest) sigma =
            if f=g
            then main ((LP.map (fn (x,y) => (x,y)) (ts,us))@rest) sigma
            else NONE
          | main ((T.Fun _, T.Var _)::_) _  = NONE
    ...
    let fun main [] sigma = SOME sigma
          | main ((T.Var x, t)::rest) sigma = (case (T.Var x)=t 
                                                of true => main rest sigma
                                                 | false => (if (L.exists (fn y => y=x) (T.vars t))
                                                             then NONE 
...
fun unionCont ([],ys) term = ys
  | unionCont (((c1,c2,t,sigma)::xs),ys) term =
    let
        ...
        fun addCont ((c1,c2,t,sigma), ys) t1 = if memberCont ((c1,c2,t,sigma), ys) t1
                                               then ys
                                               else L.rev ([(c1,c2,t,sigma)]@ys)
    in
...
fun difMatch (t1,t2) =
    let
        ...
        fun erase (c1,c2,r,sigma) = c1 (c2 r)
        fun toSigma (c1,c2,r,sigma) = sigma
        fun checkDm (s,t) (r as (c1,c2,r1,sigma)) = ((erase r = t) andalso (skel r = apply sigma s))
        fun unionSigma dmi =
            let
        ...
        fun unionC ([],ys) term = ys
          | unionC (((c1,c2,t)::xs),ys) term =
            let
                ...
                fun addCont ((c1,c2,t), ys) t1 = if memberCont ((c1,c2,t), ys) t1
                                                 then ys
                                                 else L.rev ([(c1,c2,t)]@ys)
            in
                if memberCont ((c1,c2,t),ys) term
                then unionC (xs,ys) term
                else unionC (xs, addCont ((c1,c2,t), ys) term) term
            end
    in 
        if size (erase (c1,c2,r), (c2 r)) = size (erase (c1',c2',r'), (c2' r'))
