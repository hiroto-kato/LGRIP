...
(* divergence critic *)
(* (l1->r1:ty1), (l2->r2:ty2)に対してdifference matchingを適用し、補題を生成する *)
...
(*	    val intpt = LU.union ((LU.intersection (ptl,ptr)),(L.map (fn x => Dm_ms.wavehole x ) dml1))*)
...
(*	    val _ = L.map (fn (x,y) => print ("(" ^ (T.toString x) ^ "," ^ (T.toString y) ^ ")\t")) table
	...
	in
	    if (c1 (c2 r) = r) andalso (c1' (c2' r') = r') then (r,r') (*raise noDifferenceMatch*)
	...
	in
      ...
      val dml1 = Dm_ms.MaxDm dmleft1 nil  (* maximal difference matching of left hand *)
      ...
      val dmr1 = Dm_ms.MaxDm dmright1 nil  (* maximal difference matching of right hand *)
