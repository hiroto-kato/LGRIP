...
fun pos t = 
  let
	...
	| posList ((Fun (f,ts))::nil) l = if (L.last l)=1 then [l]@(posList ts (l@[1]))
    	     	   			  else [l]@(posList ts (l@[(L.last l)-1]))
						       
	| posList ((Fun (f,ts))::ls) l = if (L.hd l)=1 then [l]@(posList ts (l@[1]))@(posList ls (L.rev (L.drop((L.rev l),(L.last l)))@[(L.last l)+1]))
...
(* t(p)を返す *)
fun symbol (Var x) l = if l=nil then Symbol.V x
...
and symbolList (t::nil) (l::ls) = symbol t ls
    | symbolList (t::ts) (l::ls) = if l=1 then symbol t ls
      		 	 	   else symbolList ts ([l-1]@ls)

(* t|pを返す *)
fun subterm (Var x) l = if l=nil then Var x
			else raise PositionNotInTerm
  | subterm (Fun t) nil = Fun t
  | subterm (Fun (f,(t::ts))) (l::ls) = case l of 
      	      	   			    1 => subterm t ls
					  | 0 => raise PositionNotInTerm
      	      	   	   	     	  | _ => if ts=nil then subterm t ([l-1]@ls) else subtermList ts ([l-1]@ls)
and subtermList (t::nil) (l::ls) = subterm t ls
  | subtermList (t::ts) (l::ls) = if l=1 then subterm t ls
...
| makeContext (Fun (f,(t::ts))) (p::ps) tc = if p=1 then Fun (f,(makeContext t ps tc)::ts)
					     else Fun (f,t::(makeContextL ts ([p-1]@ps) tc)::nil)
| makeContext _ _ tc = raise PositionNotInTerm					
and makeContextL (t::ts) (p::ps) tc = if p=1 then makeContext t ps tc
...
fun isConstructor cfuns term =
  ...
  in
      if tfuns = nil then true 
      else (if (L.filter (fn x => not(LU.member x cfuns)) tfuns) = nil then true else false)
  end
      
(* 項が基本項か判定する関数 *)
fun isBasic (dfuns, cfuns) term = if isFun term then
				      (LU.member (valOf(dfun term)) dfuns) andalso (L.all (fn t => isConstructor cfuns t) (args term))
				  else false

(* 項が階層項か判定する関数 *)
fun isHierarchy (dfuns, cfuns) term = if isBasic (dfuns,cfuns) term then true
				  else
				      case dfun term of
...
fun findBasicSubtermLiOc (dfuns, cfuns) term =
  let
	...
	| lipos [(t0,p0)] ((t1,p1)::tps) = if (L.length p0) > (L.length p1) then lipos [(t0,p0)] tps
...
fun termMatchpos term1 term2 =
  ...
  in
      if tpos2=nil then nil else (L.map (fn (t,p) => p) tpos2)
  end
...
fun termMatchPosition term1 term2 =
  let
      ...
      val tpos2 = L.filter (fn (t,p) => (t = term2)) subtl
  in
      if tpos2=nil then nil else L.hd (L.map (fn (t,p) => p) tpos2)
  end
...
fun isLinear term =
  let
	...
	| vars_a (t::ts) = (varsr t)@(vars_a ts)
      fun TorF nil = true
	| TorF (t::ts) = if LU.member t ts then false
			 else TorF ts
  in
      TorF (varsr term)
  end

fun isDfunInRoot dfuns term = if isVar term then false 
