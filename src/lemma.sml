...
(* if C[s] is Constructor、we generate the lemma generalized D[C[s]]=D'[C[s]] to D[x]=D'[x] *)
...
fun lemmaGen1 (((l1,r1),ty1),((l2,r2),ty2)) (sign,dfuns,cfuns) grter n =
    let
        ...
        (* we randomly make th rhs of rule up to the depth n *)
        ...
        val lems = case tempL1 of
                     ...
                     | SOME templ => (let
                                         ...
                                         val tempLem = case tempR of
                                                           NONE => temprls
                                                         | SOME termR => (if T.isVar termR
                                                                          then temprls
                                                                          else L.filter (fn ((l,r),ty) => (T.root r) = (T.root termR)) temprls)
                                     in
...
(* randomly lemma generation method ~Debug Version~ Depth Default *)
