...
(* induction step of rewriting induction with disproof *)
...
(* induction step of rewriting induction with disproof ~Debug Version~ *)
...
(* rewriting induction with disproof *)
fun ind (sign,rs,grter) es max =
    let
        ...
        (* we skip next processing when we add lemmas *)
        val (es',hs') = if L.length es = 1
                        then delete(decompose (sign,cfuns) (simplify (rs,grter) (es,[])))
                        else (es,[])
...
(* rewriting induction with disproof ~Debug Version~ *)
