...
fun exp2 str =
  let
	...
	let
		...
		(* Difference Match手続き *)
		val lemmas = if prop = "Stopped" then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.DivCritic (sign,rs,grter) es,nil))
			     else []
		val soundlemmas = if prop = "Stopped" then L.filter (fn e => IndMain.proc2nd (sign,rs,grter,[e]) (1,max) [e]) lemmas
				  else []
...
fun exp3 str =
  let
	...
	let
		...
		val prop = IndMs.ind (sign,rs,grter) es max 
		val _ = if prop = "Stopped" then print "Rewriting induction diverges.\nLemma Generation:" else print ""
		(* 差分照合と一般化に基づく補題生成法の手続き *)
		val lemmas = if prop = "Stopped" then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.DmNewLem (sign,rs,grter) es,nil))
			     else []
		val soundlemmas = if prop = "Stopped" then L.filter (fn e => IndMain.proc2nd (sign,rs,grter,[e]) (1,max) [e]) lemmas
				  else []
...
fun exp4 str =
  let
	...
	let
	    ...
	    val _ = case prop of
			"Stopped" => (if randomlemmas = [] then print " []\n" else print ("Proved:" ^ (Trs.prEqs (MS.dropSortInMsRules lemmas0))))
		      | _ => print ""
	in
	    if n > max  then raise Stopped
	    else
...
fun exp5 str =
  let
	...
	let
	    ...
	    (* Random Lemma Generationの手続き *)
	    val randlem = case prop of
			      "Stopped" => L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.randLemGenDepth (sign,rs,grter) es 4,nil)) (* 深さ入力 *)
