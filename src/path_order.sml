...
fun lpoGt prec (term1,term2) = 
    let fun gt (T.Var _, _) = false
	  ...
	  | gt (s as (T.Fun (f,ss)), t as (T.Fun (g,ts))) = if f=g then (geqforsome ss t) orelse ((lex gt (ss,ts)) andalso (gtforall s ts))
...
fun encodeLpoGt prec (term1,term2) =
  let
	...
	| lexproc rel (x::xs,y::ys) = F.IfThenElse (rel (x, y), F.True, (if x = y then lexproc rel (xs, ys) else F.False))
	| lexproc rel _ = raise Fail "Error: lex compares lists having different length"
(*      fun lexproc rel ([],[]) = F.False
	| lexproc rel (x::xs,y::ys) = if (x=y) then lexproc rel (xs,ys) else rel (x,y)
	| lexproc rel _ = raise Fail "Error: lex compares lists having different length"*)
			    
      fun gt (T.Var _, _) = F.False
	| gt (T.Fun (f,ss), t as (T.Var _)) = geqforsome ss t
	| gt (s as (T.Fun (f,ss)), t as (T.Fun (g,ts))) = if f=g then F.Or (geqforsome ss t , F.And (lexproc gt (ss,ts), gtforall s ts))
...
fun lpqoGt prec (term1,term2) = 
  let fun gt (T.Var _, _) = false
	...
	| gt (s as (T.Fun (f,ss)), t as (T.Fun (g,ts))) =
	  if f=g then (geqforsome ss t) orelse ((qlex geq (ss,ts)) andalso (geqforall s ts))
...
fun encodeLpqoGt prec (term1,term2) =
  let
	...
	| qlexproc rel (x::nil,y::nil) = F.IfThenElse (rel (x,y), F.True, (if x=y then F.True else F.False))
	| qlexproc rel (x::xs,y::ys) = F.IfThenElse (rel (x,y), F.True, (if x=y then qlexproc rel (xs,ys) else F.False))
	| qlexproc rel _ = raise Fail "Error: lex compares lists having different length"
      fun gt (T.Var _, _) = F.False
	| gt (T.Fun (f,ss), t as (T.Var _)) = geqforsome ss t
	| gt (s as (T.Fun (f,ss)), t as (T.Fun (g,ts))) =
	  if f=g then F.Or (geqforsome ss t , F.And (qlexproc geq (ss,ts), geqforall s ts))
