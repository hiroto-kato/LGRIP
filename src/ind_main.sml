        ...
        fun lpostr (rs,lpol) = 
            let
                ...
                fun encodeOrder ((f1,w1), []) = []
                  | encodeOrder ((f1,w1), ((f2,w2)::xs)) = if w1=w2
                                                           then encodeOrder ((f1,w1),xs)
                                                           else (if w1 > w2
                                                                 then [Not (Atom (Gt (valOf (AL.find f1 table), valOf (AL.find f2 table))))]@(encodeOrder ((f1,w1), xs))
...
(* lemma proof process *)
fun proc2nd (sign,rs,grter,es) (n,max) tmp =
    if n>max then raise Stopped else 
    let
        fun toleft (x,y) = x
        val prop = IndMs.ind (sign,rs,grter) es max 
        val lemmas = if prop = "Stopped"
                     then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.lemGen (sign,rs,grter) es, nil))
                     else []
        val soundlemmas = if prop = "Stopped"
                          then L.filter (fn e => proc2nd (sign,rs,grter,[e]) (n+1,max) (LU.union (tmp,es))) lemmas
                          else []
        val randlem = if soundlemmas = []
                      then (if prop = "Stopped"
                            then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.randLemGenDef (sign,rs,grter) es,nil))
                            else [])
                      else []
                               
        val randomlemmas = if soundlemmas = []
                           then (if prop = "Stopped"
    ...
    let
        ...
        fun procSub (sign,rs,grter,es) n lem =
            let
                ...
                (* Difference Match and New Lemma Generation procedure *)
                ...
                val _ = case  prop  of
                            "Stopped" => if soundlemmas = []
                                         then print "Random Lemma Generation:"
                                         else print ""
                          | _  => print ""
                ...
                val randomlemmas = case prop of
                                       "Stopped" => (if soundlemmas = []
                                                     then L.filter (fn ((l,r),ty) => not(l=r)) (L.filter (fn e => proc2nd (sign,rs,grter,[e]) (1,max) [e]) randlem)
                                                     else [])
                                     | _  => []
                (* the number of lemmas of random lemmas generation *)
    ...
    let
        ...
        fun procSub (sign,rs,grter,es) n lem =
            let
                ...
                (* Difference Match and New Lemma Generation procedure *)
                val lemmas = if prop = "Stopped"
                             then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.lemGenPrint (sign,rs,grter) es,nil))
                             else []
                val soundlemmas = if prop = "Stopped"
                                  then L.filter (fn e => proc2nd (sign,rs,grter,[e]) (1,max) [e]) lemmas
                                  else []q
                (* desplay proved lemmas *)
                val _ = case prop of
                            "Stopped" => if soundlemmas = []
                ...
                val randlem = case prop of
                                  "Stopped" => if soundlemmas = []
                                               then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.randLemGenDefPrint (sign,rs,grter) es,nil))
                                               else []
                                | _ => []
                val randomlemmas = case prop of
                                       "Stopped" => (if soundlemmas = []
                                     ...
                                     | _  => []
                (* the number of lemmas of random lemma generation *)
                val _ = if randlem = [] then print ""
                        else print ("\nlemmas: "^(Int.toString (L.length (L.filter grter (MS.dropSortInMsRules randlem))))^"\n")
                (* desplay proved lemmas *)
                (*val _ = case prop of
...
(* we don't use lemmas to prove lemmas: Main Process *)
    ...
    let
        ...
        fun procSub (sign,rs,grter,es) n lem =
            let
                ...
                val soundlemmas = if prop = "Stopped"
                                  then L.filter (fn e => (IndMs.ind (sign,rs,grter) [e] max) = "Success") lemmas
                                  else []
                (* desplay proved lemmas *)
                val _ = case prop of
                            "Stopped" => if soundlemmas = []
                ...
                val randomlemmas = case prop of
                                       "Stopped" => (if soundlemmas = []
                                                     then L.filter (fn ((l,r),ty) => not(l=r)) (L.filter (fn e => (IndMs.ind (sign,rs,grter) [e] max) = "Success") randlem)
                                                     else [])
                                     | _  => []
                (* the number of lemmas of random lemma generation *)
                val _ = if randlem = [] then print ""
...
(* we don't use lemmas to prove lemmas: Main Process-Default *)
fun proc_NolemgenDef str = proc_Nolemgen str 6
           
(* we don't use lemmas to prove lemmas: ~Debug Version~ *)
...
(* we don't use lemmas to prove lemmas: ~Debug Version~ Default*)
fun proc_NolemgenPrintDef str = proc_NolemgenPrint str 6

(* lemmas proof process(depth input) *)
fun procDepth2nd (sign,rs,grter,es) (n,max) tmp depth =
    if n>max then raise Stopped
    else 
        let
            fun toleft (x,y) = x
            val prop = IndMs.ind (sign,rs,grter) es max 
            val lemmas = if prop = "Stopped"
                         then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.lemGen (sign,rs,grter) es, nil)) (*LemGen.lemGenNoPrint (sign,rs,grter) es*)
                         else []
            val soundlemmas = if prop = "Stopped"
                              then L.filter (fn e => procDepth2nd (sign,rs,grter,[e]) (n+1,max) (LU.union (tmp,es)) depth) lemmas
                              else []
            val randlem = if soundlemmas = []
                          then (if prop = "Stopped"
                                then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.randLemGenDepth (sign,rs,grter) es depth,nil))
                                else [])
                          else []
            val randomlemmas = if soundlemmas = []
                               then (if prop = "Stopped"
    ...
    let
        ...
        fun procSub (sign,rs,grter,es) n lem =
            let
                ...
                val soundlemmas = if prop = "Stopped"
                                  then L.filter (fn e => procDepth2nd (sign,rs,grter,[e]) (1,max) [e] depth) lemmas
                                  else []
                (* desplay proved lemmas *)
                val _ = case prop of
                            "Stopped" => if soundlemmas = []
                ...
                val randomlemmas = case prop of
                                       "Stopped" => (if soundlemmas = []
                                                     then L.filter (fn ((l,r),ty) => not(l=r)) (L.filter (fn e => procDepth2nd (sign,rs,grter,[e]) (1,max) [e] depth) randlem)
                                                     else [])
                                     | _  => []
                (* the number of lemmas of random lemma generation  *)
                val _ = if randlem = [] then print ""
    ...
    let
        ...
        fun procSub (sign,rs,grter,es) n lem =
            let
                ...
                val _ = case  prop  of
                            "Stopped" => if soundlemmas = []
                                         then print "Random Lemma Generation:" else print ""
                          | _  => print ""
                (* Random Lemma Generation procedure *)
                val randlem = case prop of
                                  "Stopped" => if soundlemmas = []
                                               then L.filter (fn ((l,r),ty) => not(l=r)) (MS.diffMsRules (LemGen.randLemGenDepthPrint (sign,rs,grter) es depth,nil))
                                               else []
                                | _ => []
                val randomlemmas = case prop of
                                       "Stopped" => (if soundlemmas = []
...
(* we don't use lemmas to prove lemmas:(depth input) Main Process *)
...
(* we don't use lemmas to prove lemmas:(depth input) ~Debug Version~ Default*)
fun procDepth_NolemgenPrintDef str depth =  procDepth_NolemgenPrint str 6 depth

(* display only lemmas (depth is lemmas of lhs) *)
fun procLemmas str =
    let
        val (sign0,rs0,grter0,es0) = indPars str
        val max = 6
...
(* desplay only lemmas (input depth) *)      
