...
(* divergence critic *)
(* (l1->r1), (l2->r2)に対してdifference matchingを適用し、補題を生成する *)
fun dc ((l1,r1),(l2,r2)) cfuns =
  let
	...
	| MaxDm ((c1,c2,r)::xs) (ts as ((c1',c2',r')::ys)) = if size (erase (c1,c2,r), (c2 r)) = size (erase (c1',c2',r'), (c2' r')) then MaxDm xs (unionC (ts@[(c1,c2,r)],nil) term)
...
(*	    val _ = L.map (fn (x,y) => print ("(" ^ (T.toString x) ^ "," ^ (T.toString y) ^ ")\t")) table
	...
	in
      ...
      fun makerule (c1,c2,r) (c1',c2',r') = if (c1 (c2 r) = r) andalso (c1' (c2' r') = r') then (r,r') 
					    else (if erase (c1',c2',r') = r' then (erase (c1,c2,r), c1 r)
...
*)		    
      ...
      val rules = L.concat (LP.map (fn (dml,dmr) => if (skel dml = wavehole dml) then [makerule dmr dml]
...
fun dc1 ((l1,r1),(l2,r2)) =
  let
	...
	| MaxDm ((c1,c2,r)::xs) (ts as ((c1',c2',r')::ys)) = if size (erase (c1,c2,r), (c2 r)) = size (erase (c1',c2',r'), (c2' r')) then MaxDm xs (unionC (ts@[(c1,c2,r)],nil) term)
      ...
      (* differenceMatchではないものをdelete : (Term.Context * Term.Context * Term.term) list -> (Term.Context * Term.Context * Term.term) list *)
      fun deleteNoDm ts =
	let
	    val d = case (L.filter (fn t1 => not(erase t1 = wavehole t1)) ts) of 
			nil => [hole]
...
(*      val _ = print (Trs.prRules [(l1,r1),(l2,r2)])
	...
	in
      ...
      val rules1 = (L.concat (L.map (fn dml => L.concat (L.map (fn dmr=> (if (L.length (T.args (erase dml)) = 1) andalso (skel dml = wavehole dml) then []
